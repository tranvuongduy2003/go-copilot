...
func TestDeactivateUserHandler_Handle(t *testing.T) {
	...
	tests := []struct {
		...
		command     func(*user.User) DeactivateUserCommand
		wantErr     bool
		errContains string
	...
	}{
		{
			...
			command: func(u *user.User) DeactivateUserCommand {
			...
			checkResult: func(t *testing.T, userRepo *testutil.MockUserRepository, eventBus *testutil.MockEventBus, u *user.User) {
				...
				assert.Equal(t, "user.deactivated", eventBus.PublishedEvents[0].EventType())
		...
		{
			name: "fail when user is already inactive",
			setupMocks: func(userRepo *testutil.MockUserRepository, eventBus *testutil.MockEventBus, u *user.User) {
				u.Deactivate()
				u.ClearDomainEvents()
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := NewDeactivateUserHandler(userRepo, eventBus, logger)
